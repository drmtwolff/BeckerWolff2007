...
%%
%%  Program: sim2phys.c
%%  Purpose: Take the input parameters values of the BW similarity variables that 
%%           have the Greek letter names and convert those to physical variable 
%%           parameter input values.  Either implementation can fully specify 
%%           the BW spectral model flow parameters. 
%%
...
**	Parse command line:
*/
	while ( ( c=getopt(argc,argv,"hva:b:c:d:l:m::p::r::t:x:") ) != -1 ) {
		switch(c) {
...
/* 				if ( b12==0.00 ) printhelp=1; */
			...
			case 'd': /* REQUIRED Source distance, units kiloparsecs  */
				...
				break;
			case 'l': /* REQUIRED BW2007 "delta" similarity variable */
				delta=strtod(optarg,&convertablenumber);
				if ( optarg==convertablenumber ) printhelp=1;
 				if ( delta==0.00 ) printhelp=1;
				break;
			...
			case 'x': /* REQUIRED BW2007 "xi" similarity variable */
				xi=strtod(optarg,&convertablenumber);
				if ( optarg==convertablenumber ) printhelp=1;
 				if ( xi==0.00 ) printhelp=1;
				break;
...
*/
	if ( xi<=0.0 ) printhelp=1;
	if ( delta<=0.0 ) printhelp=1;
...
*/
	if ( printhelp==1 ) {
		fprintf(stderr," Usage: sim2phys [hva:b:c:d:l:m::p::r::t:x:] \n");
		...
		fprintf(stderr,"\t-l\tDelta (unitless).\n");
...
*/
	fprintf(stdout,"\n");
	fprintf(stdout," SIM2PHYS Version: 1.48 Date: 2025/05/23\n");
	fprintf(stdout,"\n");
...
/*
**	Go from My system of physical variables to Pete's system of similarity (Greek) variables:
*/
	sigpara=(caprad*caprad/(acrat*acrat*xi*xi))*(M_PI*M_PI*protonmass*protonmass*speedlight*speedlight/(sigperp));
	alpha=xi*(32.0*sqrt(3.0)/(49.0*log(7.0/3.0)))*(Grav*NSmass/(NSRadius*speedlight*speedlight));
	sigbar=(alpha/3.0)*(sigpara/delta)*(electronmass*speedlight*speedlight/(kboltz*comptem));
